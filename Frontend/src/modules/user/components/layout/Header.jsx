...
import { FiSearch, FiShoppingCart, FiSend } from 'react-icons/fi';
  ...
  const handleTestPush = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast.error(result.error || 'Failed to send test push notification', {
          id: toastId,
          duration: 5000
        });
      }
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-3.5 sm:px-4 py-2.5 sm:py-3 flex items-center justify-between gap-2.5 sm:gap-3">
        ...
        <div
          className="flex flex-col cursor-pointer max-w-[45%] xs:max-w-[52%] sm:max-w-md group min-w-0"
        ...
        {/* Right Side: Test Push, Search, Notification & Cart Button */}
        <div className="flex items-center gap-1.5 sm:gap-2.5 shrink-0">
          ...
          <button
            ...
            className="px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-full bg-white/20 hover:bg-white/30 active:scale-95 text-white flex items-center gap-1.5 transition-all duration-200 backdrop-blur-sm border border-white/25 shadow-xs cursor-pointer disabled:opacity-50 text-[11px] sm:text-xs font-bold"
            title="Tap to test push notifications"
          >
            {isTestingPush ? (
              <div className="w-3 h-3 border-2 border-white/30 border-t-white rounded-full animate-spin" />
            ) : (
              <FiSend className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-white" />
            )}
            <span>{isTestingPush ? 'Testing...' : 'Test Push'}</span>
          </button>
